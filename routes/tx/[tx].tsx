...
export const handler: Handlers<Transaction | null> = {
  async GET(_, ctx) {
    let tx = decodeURI(ctx.params.tx);
    let address = Address.parse(tx.split("|")[0]);
    let lt = tx.split("|")[1];
    let hash = Buffer.from(tx.split("|")[2], "base64");
    
    let postBody = {"id":"1","jsonrpc":"2.0","method":"getTransactions","params":{"address":address.toString(),"limit":1, "hash":hash.toString("hex"), lt: lt  }};
...
export default function TransactionDetailed({ data, params }: PageProps<Transaction[] | null>) {
  ...
  return (
    <div class={tw`p-4 mx-auto max-w-screen-md`}>
      <p class={tw`my-2 text-2xl m-4 font-medium`}>
        <a class={tw``} href={`/address/${currentContract}`}>👾{currentContract}</a>
      </p>
    <p class={tw`my-2 text-6xl m-4 font-medium`}>Transaction</p>
     <div class={tw`grid grid-cols-4 gap-4 content-start bg-white border-b dark:bg-gray-800 dark:border-gray-700`}>
...
function Tx(element, txData) {
  return (
    <div class={tw`bg-white border-b dark:bg-gray-800 dark:border-gray-700 p-1 `}>
...
function Actions(data: Array<any>, txData) {
  ...
  let list = data.map( (element, i) => {
    ...
    let value = fromNano(action.info.value.coins.toString());
      ...
      return (<div class={tw`grid grid-cols-3 content-start p-2  border-t`}>
