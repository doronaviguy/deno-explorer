...
export const handler: Handlers<Transaction | null> = {
  async GET(_, ctx) {
    ...
    let address = Address.parse(ctx.params.name);
...
export default  function Transactions({ data, params }: PageProps<Transaction[] | null>) {
  ...
  return (
    ...
    <p class={tw`my-2 text-lg font-medium`}>
      Address {params.name} Transactions
      </p>
...
function Tx(element) {
  return (
    <div class={tw`bg-white border-b dark:bg-gray-800 dark:border-gray-700 p-1 `}>
      <div class={tw`grid grid-cols-1 content-end space-y-3`}>
...
function Actions(data: Array<any>) {
    ...
    return (<div class={tw`grid grid-cols-3 content-start p-2  border-b`}>
